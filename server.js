...
app.post('/claim/ask', requireAuth, async (req, res) => {
  try {
    ...
    const system = [
      'FACTUAL ACCURACY RULES (these override anything you infer): (1) amount_paid_to_date in the claim data is the ONLY figure that means money actually received - if it is 0 or absent, the carrier has PAID NOTHING; never write that they paid, issued a cheque, or tendered funds. (2) An offer or carrier_offer_on_file value is a number put on the table, not money received. (3) Before asking the carrier for any document, check documents_on_file in the claim data - if it is already listed, do NOT request it; refer to it as already provided. (4) If no demand amount is recorded, say the demand amount is not recorded in the file - do NOT assert that no demand was ever made. (5) Never invent figures, dates, or documents. FACTUAL ACCURACY RULES (these override anything you infer): (1) amount_paid_to_date in the claim data is the ONLY figure that means money actually received - if it is 0 or absent, the carrier has PAID NOTHING; never write that they paid, issued a cheque, or tendered funds. (2) An offer or carrier_offer_on_file value is a number put on the table, not money received. (3) Before asking the carrier for any document, check documents_on_file in the claim data - if it is already listed, do NOT request it; refer to it as already provided. (4) If no demand amount is recorded, say the demand amount is not recorded in the file - do NOT assert that no demand was ever made. (5) Never invent figures, dates, or documents. You are a senior public adjuster helping the user work a property insurance claim.',
...
// The prompt builders render the claim via a fixed label map, which silently DROPS any key
// not listed. These structured fields matter for accuracy, so append them explicitly.
function _extraClaimCtx(claim) {
  if (!claim || typeof claim !== 'object') return '';
  var out = [];
  try {
    if (claim.amount_paid_to_date != null) {
      var paid = Number(claim.amount_paid_to_date) || 0;
      out.push('MONEY ACTUALLY RECEIVED to date: headers, name) {
  if (!headers) return '';
...
app.post('/emails/:msgId/draft-reply', requireAuth, async (req, res) => {
  try {
    ...
    if (!_money || _money.demand == null) { _mLines.push('OUR DEMAND: not recorded in this file. Do NOT present any other figure (especially the carrier offer) as our demand, and do not invent one.'); }
...
app.post('/claim/work', requireAuth, async (req, res) => {
  try {
    ...
    var systemPrompt = [
      "When FOCUS DOCUMENTS are supplied, read them closely and let them drive your analysis: quote their own wording, attack unsupported causation findings, list internal contradictions and gaps, and cross-check them against the estimate/money on the claim. FACTUAL ACCURACY RULES (these override anything you infer): (1) amount_paid_to_date in the claim data is the ONLY figure that means money actually received - if it is 0 or absent, the carrier has PAID NOTHING; never write that they paid, issued a cheque, or tendered funds. (2) An offer or carrier_offer_on_file value is a number put on the table, not money received. (3) Before asking the carrier for any document, check documents_on_file in the claim data - if it is already listed, do NOT request it; refer to it as already provided. (4) If no demand amount is recorded, say the demand amount is not recorded in the file - do NOT assert that no demand was ever made. (5) Never invent figures, dates, or documents. You are a senior public adjuster's assistant working a single insurance claim. Read the claim data and the full email trail, then produce a concrete work-up that does as much of the work as possible. Your job is to get the policyholder the most money possible. Read the claim data, the MONEY/settlement position, OUR estimate, the adjuster instruction, and the full email trail, then produce a concrete work-up that does as much of the work as possible. You advocate hard for the policyholder against the carrier. CRITICAL DISTINCTION: the MONEY / SETTLEMENT POSITION section holds the CARRIER's estimate and offers (their numbers, entered in Demands). OUR estimate is a SEPARATE document shown under OUR ESTIMATE (in our files). Never confuse the two. If a carrier estimate/offer is listed in the MONEY section, the carrier DID send their estimate - never say they did not. If OUR ESTIMATE says none was found, we simply have not uploaded ours yet.",
      '',
...
Return ONLY a JSON object with these keys (use null when a value is not clearly shown):
{
  ...
  "claim_number": "claim number if shown, else null",
    "policy_number": "policy number if shown, else null",
    "insured": "the insured / policyholder / customer / owner name if shown, else null",
    "property_address": "the loss or property address if shown, else null",
    "date_of_loss": "the date of loss / date of damage / DOL as YYYY-MM-DD if shown (NOT the estimate print date), else null",
  ...
  "roof_depreciation": "the recoverable depreciation withheld on the ROOF / roof-covering line items ONLY (the roof section of the estimate), number only. Null if the estimate does not separate roof from interior.",
  "interior_depreciation": "the recoverable depreciation withheld on INTERIOR / dwelling / non-roof line items (everything that is not the roof), number only. Null if the estimate does not separate roof from interior.",
  "ordinance_law": "the total Ordinance or Law (also Ordinance & Law, Ord & Law, Code Upgrade, Building Code Upgrade) coverage amount, number only. Null if none.",
...
app.post('/policy/extract', requireAuth, async (req, res) => {
  try {
    const { image_base64, media_type, pdf_base64, text: policyText, filename, claim_local_id } = req.body || {};
    if (!image_base64 && !pdf_base64 && !policyText) {
    ...
    const content = [];
    if (policyText) {
      content.push({ type: 'text', text: 'POLICY TEXT extracted from the PDF in the browser (Declarations page first; sections mentioning appraisal, searched across the entire policy, follow):' + String.fromCharCode(10, 10) + String(policyText).slice(0, 60000) });
    } else if (pdf_base64) {
      content.push({
        type: 'document',
...
boot();
 + paid + (paid > 0 ? '' : '  <-- the carrier has PAID NOTHING so far'));
    }
    if (claim.carrier_offer_on_file != null) {
      out.push('Carrier offer sitting on the table: headers, name) {
  if (!headers) return '';
  const h = headers.find(x => (x.name || '').toLowerCase() === name.toLowerCase());
  return h ? (h.value || '') : '';
}
...
boot();
 + claim.carrier_offer_on_file + '  <-- this is THEIR offer. It is NOT money received and it is NOT our demand.');
    }
    if (Array.isArray(claim.documents_on_file) && claim.documents_on_file.length) {
      out.push('DOCUMENTS ALREADY ON FILE (never ask for these to be sent or resent - they exist):');
      claim.documents_on_file.forEach(function (d) {
        out.push('  - ' + (d && d.name ? d.name : '') + (d && d.folder ? ('  [' + d.folder + ']') : ''));
      });
    }
    if (Array.isArray(claim._data_rules) && claim._data_rules.length) {
      out.push('DATA RULES YOU MUST FOLLOW:');
      claim._data_rules.forEach(function (r) { out.push('  - ' + r); });
    }
  } catch (e) {}
  return out.length ? (String.fromCharCode(10) + out.join(String.fromCharCode(10))) : '';
}
