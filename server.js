...
function _extraClaimCtx(claim) {
  ...
  try {
    if (claim.amount_paid_to_date != null) {
      ...
      out.push('MONEY ACTUALLY RECEIVED to date: USD ' + paid + (paid > 0 ? '' : '  <-- the carrier has PAID NOTHING so far'));
    }
    if (claim.carrier_offer_on_file != null) {
      out.push('Carrier offer on the table: USD ' + claim.carrier_offer_on_file + '  <-- THEIR offer. Not money received, and NOT our demand.');
    }
...
function _ltrResolve(text, data){
  ...
  if(text!=null) text = _ltrSafe(text);
  if(text==null) return '';
  let out='', i=0;
  while(i<text.length){
    if(text[i]==='['&&text[i+1]==='['){
